...
3. **Environment Variables** (marque _Production_ e _Preview_):
   ...
   | `NEXT_PUBLIC_APP_URL`                  | `https://app.tivexy.com.br`                                            |

   `OPENAI_API_KEY` fica **de fora** por enquanto (a IA não existe ainda).

4. **Deploy**. ✅ Termina **Ready**.
5. **Settings → Domains → Add**, um de cada vez:
   - `app.tivexy.com.br`
   - `*.tivexy.com.br`
...
3. **Redirect URLs → Add URL**, uma por vez:
   ...
   - `https://*.tivexy.com.br/**`
   - (desenvolvimento, se quiser manter) `http://localhost:3000/**`
4. **Save**.
